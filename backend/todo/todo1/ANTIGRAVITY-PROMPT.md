...
| `Hero` | `src/components/site/Hero.tsx` | Full-bleed sky gradient (`#7CC0F5 → #007AE5 → #1C3F99`). **Scroll-linked animations (REQUIRED):** (a) hero section scales 1 → 1.05 + drifts up 60px as user scrolls; (b) plane flies DOWN-RIGHT (x: 0→600, y: 0→380, rotate: -45→-15, opacity fades near end); (c) contrail grows scaleX 1→2.4 + fades; (d) clouds parallax up (slower than text) + fade; (e) text content parallaxes up (y: 0→-180) + fades; (f) search form lifts faster (y: 0→-220) + fades; (g) scroll hint fades out immediately on first 15% scroll. Use framer-motion's `useScroll({ target: heroRef, offset: ["start start", "end start"] })` + `useTransform`. **CSS background:** animated clouds (3 blur blobs with `animate-drift-1/2/3`), sun glow blob. **Foreground:** floating plane with contrail (scroll-linked), rotating headline every 3.8s ("Skip traffic. Time to fly." variants, AnimatePresence fade+slide using `headlineSwap` variant). Embeds `SearchForm`. Stats strip with 4 numbers. |
...
- **Hero scroll-linked animations (REQUIRED, signature feel):** The hero section MUST use `useScroll({ target: heroRef, offset: ["start start", "end start"] })` to drive these scroll-linked transforms via `useTransform`:
  - **Hero section:** `scale: [1, 1.05]`, `y: [0, -60]` — subtle zoom + lift as it leaves
  - **Plane:** `x: [0, 600]`, `y: [0, 380]`, `rotate: [-45, -15]`, `opacity: [1, 0.8, 0]` — flies down-right and fades
  - **Contrail:** `scaleX: [1, 2.4]`, `opacity: [0.8, 0.6, 0]` — grows longer as plane flies, fades near end
  - **Clouds (parallax background):** `y: [0, -120]`, `opacity: [1, 0.7, 0]` — slower than text, fades out
  - **Text content:** `y: [0, -180]`, `opacity: [1, 0.4, 0]` — parallaxes up + fades
  - **Search form:** `y: [0, -220]`, `opacity: [1, 0.6, 0]` — lifts away first (faster parallax)
  - **Scroll hint:** `opacity: [1, 0]` over scroll range `[0, 0.15]` — fades out immediately on first scroll
  - Use the helper hooks from `joby-motion.ts`: `usePlaneScroll`, `useContrailScroll`, `useCloudsScroll`, `useHeroTextScroll`, `useSearchFormScroll`, `useHeroSectionScroll`, `useScrollHintScroll` — each takes a `MotionValue<number>` (your `scrollYProgress`) and returns the transforms.
  - All scroll-linked motion layers must have `will-change-transform` Tailwind class for smooth GPU acceleration.
  - The plane's gentle bobbing (`planeFloat` variant, `y: [0, -8, 0]` over 4s infinite) must be layered ON TOP of the scroll-driven motion via a nested motion div — the scroll drives the big motion, the float drives the micro-motion.
...
3. **Scroll the home page → the plane flies down-right and fades out, the hero text parallaxes up + fades, the clouds drift at parallax speed, the search form lifts away first.** (Verified via Agent Browser: scroll 700px → plane at x≈1165, y≈-304, opacity 0.59; H1 opacity 0.18.)
4. Search "JFK → LHR" returns ≥ 1 flight.
5. Clicking "Select" on a flight opens the booking wizard overlay with a live seat map (6-abreast, cabin switcher).
6. Picking a seat shows the price in a sticky sidebar and enables "Continue".
7. After passenger info + payment, `POST /api/booking` returns 201 with a `SKY-XXXXX` reference.
8. Confirmation screen shows a boarding pass with the reference + barcode.
9. Mobile (390×844): navbar collapses to hamburger, hero is full-bleed, all sections stack.
10. Every animation uses `JOB_EASE`. Every link uses `nav-underline`. Every button is `rounded-full`.
