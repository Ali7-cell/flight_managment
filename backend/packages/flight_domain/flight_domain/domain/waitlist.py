...
def join_waitlist(
    ...
    *,
    ...
    # Basic economy only exists on Economy cabin
    if seat_class and seat_class.class_name.lower() in ("business", "first"):
        fare_type = "flex"
...
def get_waitlist_position(
    ...
    """
    Computes position fresh at query time per policy rules:
    1. Loyalty tier: Platinum > Gold > Silver > none
    2. Fare type: flex before basic_economy
    3. Time joined: earlier first
    """
    ...
    # Fetch all waiting candidates for this seat class and rank dynamically
    candidates = (
        session.query(WaitlistEntry, Passenger)
    ...
    tier_order = {"platinum": 1, "gold": 2, "silver": 3, "none": 4}
    fare_order = {"flex": 1, "basic_economy": 2}

    def candidate_key(item):
        w_entry, p_entry = item
        t_rank = tier_order.get((p_entry.loyalty_tier or "none").lower(), 4)
        f_rank = fare_order.get((getattr(w_entry, "fare_type", "flex") or "flex").lower(), 2)
        return (t_rank, f_rank, w_entry.created_at)

    sorted_candidates = sorted(candidates, key=candidate_key)
    position = 1
    for idx, (c_entry, _) in enumerate(sorted_candidates, start=1):
        if c_entry.id == entry.id:
            position = idx
            break
