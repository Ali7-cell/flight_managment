...
def confirm_booking(
    ...
    *,
    ...
    4. Records Payment and Booking with validated fare_type
    ...
    fare_type_norm = (fare_type or "flex").lower()
    if fare_type_norm not in ("basic_economy", "flex"):
        raise ValueError(f"Invalid fare_type '{fare_type}'. Must be 'basic_economy' or 'flex'.")
    if fare_type_norm == "basic_economy" and seat_class.class_name.lower() != "economy":
        raise ValueError(f"basic_economy fare type is only valid for Economy seat class, got {seat_class.class_name}")
...
def cancel_booking(
    ...
    *,
    ...
    is_airline_initiated = actor in ("airline", "ops", "system:schedule_change") or (
        reason and "airline" in reason.lower()
    )

    if fare_type == "basic_economy":
        # Finalized rule: basic_economy cannot be cancelled for cash refund.
        # Voluntary: no refund and no credit.
        # Airline-initiated: 100% travel credit (12-month validity), never cash refund.
        if is_airline_initiated:
        ...
        # Finalized rule for flex:
        # 72+ hours before departure: 100% refund
        # 24-72 hours before departure: 75% refund
        # Under 24 hours before departure: 50% refund
        # No-show: 0% refund
        # At any tier, passenger may choose 100% travel credit instead.
        flight = session.get(Flight, booking.flight_id)
        pct = Decimal("1.0")
        if flight and flight.departure_at:
            dep_dt = (
                flight.departure_at.replace(tzinfo=timezone.utc)
                if flight.departure_at.tzinfo is None
                else flight.departure_at
            )
            now_dt = utcnow()
            hours_diff = (dep_dt - now_dt).total_seconds() / 3600.0
            if hours_diff >= 72.0:
                pct = Decimal("1.0")
        ...
        choose_credit = bool(reason and "credit" in reason.lower())
        if choose_credit:
            credit_expires_at = utcnow() + timedelta(days=365)
            credit = TravelCredit(
                passenger_id=booking.passenger_id,
            ...
            refund_amount = (booking.total_amount * pct).quantize(Decimal("0.01"))
            refund = Refund(
                booking_id=booking.id,
                amount=refund_amount,
                reason=reason or f"Flex cancellation ({int(pct * 100)}% tier)",
                status="pending",
            )
            session.add(refund)
            session.flush()
    ...
    if passenger:
        if refund_id:
            refund_str = "A refund has been initiated to your original payment method."
        elif credit_issued and credit_expires_at:
            refund_str = f"A travel credit of ${credit_issued} valid until {credit_expires_at.strftime('%Y-%m-%d')} has been issued."
        else:
            refund_str = "Under your fare rules, voluntary cancellation of Basic Economy is non-refundable with no credit issued."
        gmail_client.send(
